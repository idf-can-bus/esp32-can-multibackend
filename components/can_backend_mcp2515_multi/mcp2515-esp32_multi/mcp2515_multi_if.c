...
static struct can_dev_handle_s s_dev_handles[CANIF_MAX_BUSES]; // one default per bus for indexing fast-path
...
static void ensure_handles_built(void)
{
    for (size_t i = 0; i < s_bus_count; ++i) {
        s_bus_handles[i].bundle = s_bundles[i];
        s_dev_handles[i].bundle = s_bundles[i];
        s_dev_handles[i].device_index = 0;
    }
}
...
can_dev_handle_t canif_device_at(can_bus_handle_t bus, size_t index)
{
    ...
    static struct can_dev_handle_s handle; // transient per-call; safe for immediate use
    handle.bundle = bus->bundle;
...
can_dev_handle_t canif_dev_get_by_id(can_bus_id_t bus_id, can_dev_id_t dev_id)
{
    ...
    for (size_t i = 0; i < bus->bundle->device_count; ++i) {
        if (bus->bundle->devices[i].dev_id == dev_id) {
            static struct can_dev_handle_s handle; // transient per-call
            handle.bundle = bus->bundle;
...
bool canif_open_device(can_dev_handle_t dev)
{
    ...
    if (mcp2515_spi_init_bus_if_needed(host, &bus_cfg) != ESP_OK) return false;
...
static void event_trampoline(MCP2515_Handle h, uint32_t mask, void* user)
{
    ...
    // We cannot provide a stable device handle pointer yet; pass NULL for now.
    bind->user_cb(NULL, mask, bind->user_data);
}
...
bool canif_multi_init_default(const mcp2515_bundle_config_t* cfg)
{
    ...
    return canif_open_device(canif_device_default());
}

bool canif_multi_deinit_default(void)
{
    return canif_close_device(canif_device_default());
}
